...
if __name__ == "__main__":
    ...
    parser.add_argument("--cid", help="condor cluster id", default=500, type=int)
    ...
    parser.add_argument("--num_gpus", help="number of GPUs to use", default=1, type=int)
    parser.add_argument("--num_workers", help="number of workers", default=2, type=int)
    parser.add_argument("--num_gpus_per_worker", help="number of GPUs per worker", default=0, type=float)
    parser.add_argument("--seed", help="random seed", default=100, type=int)
    parser.add_argument("--env", help="environment name", default='smacv2', type=str)
    parser.add_argument("--map", help="map name", default='terran_5_vs_5', type=str)
    parser.add_argument("--algo", help="algorithm name", default='acppo', type=str)
    parser.add_argument("--hyperparam_source", help="test/$ENV/common", default='smac', type=str)
    parser.add_argument("--local_mode", help="run in local mode", action='store_true', default=False)
    parser.add_argument("--local_dir", help="local directory", default='/workspace/experiments', type=str)
    parser.add_argument("--exp_name", help="experiment name", default='test', type=str)
    parser.add_argument("--checkpoint_freq", help="checkpoint frequency", default=10000000, type=int)
    ...
    parser.add_argument("--policy_type", help="policy type: gru or mlp", default='gru', type=str)
    ...
    algo.fit(env, model, stop={'timesteps_total': args.timesteps_total}, share_policy=args.share_policy, local_mode=args.local_mode,
            ...
            exp_name=args.exp_name, seed=args.seed, cid=args.cid, pid=args.pid, num_gpus=args.num_gpus, num_workers=args.num_workers,
